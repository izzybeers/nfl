...
bet_ids_helper = function()
{
  ...
  response = tryCatch({
    ...
    print(e$message)
    message("DK category request failed")
    return(NULL)
  })
  if(is.null(response))
  {
  ...
  if (status_code(response) == 200)
  {
    ...
    all_categories = categories %>%
      ...
      mutate(response_var_root = case_when(
        ...
        str_detect(tolower(name), 'rec') & str_detect(tolower(name),'rush') ~ 'rushing_receiving_yards',
...
function()
{
  return(bet_ids_helper())
}

lines_by_bet_id_helper = function(bet_id)
{
...
function(bet_id)
{
  return(lines_by_bet_id_helper(bet_id))
}