...
for file in Path('./ml_ready_data/train/').glob('*.parquet'):
    ...
    if response not in set(comparisons_table['response_var']):
      print(f"Beginning model tuning for {response}")
      train = pd.read_parquet(file)
      test = pd.read_parquet(Path(f"./ml_ready_data/test/{response}.parquet"))
      train = train.drop(columns = ['gsis_id', 'team', 'display_name', 'game_id', 'season', 'jersey_number'], errors = 'ignore')
      ...
      test = test.drop(columns = ['gsis_id', 'team', 'display_name', 'game_id', 'season', 'jersey_number'], errors = 'ignore')
...
#       #XGBOOST:
      ...
      joblib.dump(xgb_dict['uncalibrated_model'], xgb_folder / 'uncalibrated_model.joblib')
    ...
    # categorical columns may have been dropped inside run_gbm).
      ...
      {
          'response_var': response,
          'num_columns': len(gbm_feature_columns),
          'model_type': 'gbm',
          'extra_calibration': False,
      ...
      neural_net_uncalibrated_model, neural_net_calibrated_model, neural_net_best_params, preprocessing_artifacts, X_train_standardized, X_test_standardized = run_neural_net(X_train, X_test, Y_train, Y_test)
      ...
      joblib.dump(neural_net_best_params, neural_net_folder / "selected_params.joblib")
...
survivors = comparisons_table_with_baseline_log_loss[(comparisons_table_with_baseline_log_loss['calibration_score'] <= 0.06) & (comparisons_table_with_baseline_log_loss['pct_bad'] <= 0.3) & (comparisons_table_with_baseline_log_loss['pct_improvement_over_baseline']>0)]

survivors = survivors.assign(
    range_calibrations=lambda x:
        x[['calibration_score_under_30', 'calibration_score_30_to_60', 'calibration_score_above_60']].max(axis=1)
        -
        x[['calibration_score_under_30', 'calibration_score_30_to_60',  'calibration_score_above_60']].min(axis=1),

    calibration_ranges_available=lambda x:
        x[['calibration_score_under_30', 'calibration_score_30_to_60', 'calibration_score_above_60']].notna().sum(axis=1)
)

def select_best_model(df, calibration_tolerance=0.01, pct_bad_tolerance=0.05, range_calibration_tolerance=0.02, log_loss_improvement_tolerance=0.02):
    candidates = df.copy()
    # 1. Overall calibration: lower is better
    best = candidates["calibration_score"].min()
    candidates = candidates[candidates["calibration_score"] <= best + calibration_tolerance].copy()
   
   # 2. pct_bad: lower is better
    if len(candidates) > 1:
        best = candidates["pct_bad"].min()
        candidates = candidates[candidates["pct_bad"] <= best + pct_bad_tolerance].copy()

    # 3. Calibration consistency across probability regions: lower is better
    if len(candidates) > 1:
        best = candidates["range_calibrations"].min()
        candidates = candidates[candidates["range_calibrations"] <= best + range_calibration_tolerance].copy()

    # 4. Validation log-loss improvement: higher is better
        if len(candidates) > 1:
            best = candidates["pct_improvement_over_baseline"].max()
            candidates = candidates[candidates["pct_improvement_over_baseline"] >= best - log_loss_improvement_tolerance].copy()

    # 5. Prefer broader calibration evidence. 3/3 > 2/2 > 1/1, but only after actual calibration quality has been considered
    if len(candidates) > 1:
        best = candidates["calibration_ranges_available"].max()
        candidates = candidates[candidates["calibration_ranges_available"] == best].copy()

    candidates = candidates.assign(
        tie_break_score=
            ((candidates["calibration_score"] - candidates["calibration_score"].min()) / calibration_tolerance)
            +
            ((candidates["pct_bad"] - candidates["pct_bad"].min()) / pct_bad_tolerance)
            +
            ((candidates["range_calibrations"] - candidates["range_calibrations"].min()) / range_calibration_tolerance)
            +
            ((candidates["pct_improvement_over_baseline"].max() - candidates["pct_improvement_over_baseline"]) / log_loss_improvement_tolerance)
    )

    candidates = candidates.sort_values(
        ["tie_break_score", "model_type", "extra_calibration"],
        ascending=[True, True, True]
    )
    # 6. Deterministic final tie-break This should almost never matter. For now use raw calibration, then log loss.
...
best_models = (survivors.groupby("response_var", group_keys=False).apply(select_best_model).reset_index())
best_models['extra_calibration'] = best_models['extra_calibration'].astype(bool)

def assess_model(row):
    tolerances = {
        "pct_improvement_over_baseline": .02,
        "calibration_score": .01,
        "range_calibrations": .02,
        "pct_bad": .05
    }

    tiers = {
        "Extra Strong": {
            "pct_improvement_over_baseline": (.25, "higher"),
            "calibration_score": (.01, "lower"),
            "range_calibrations": (.01, "lower"),
            "pct_bad": (.05, "lower")
        },
        "Strong": {
            "pct_improvement_over_baseline": (.15, "higher"),
            "calibration_score": (.02, "lower"),
            "range_calibrations": (.01, "lower"),
            "pct_bad": (.25, "lower")
        },
        "Medium": {
            "pct_improvement_over_baseline": (.05, "higher"),
            "calibration_score": (.03, "lower"),
            "range_calibrations": (.03, "lower"),
            "pct_bad": (.25, "lower")
        }
    }

    for tier, criteria in tiers.items():
        results = []
        for metric, (cutoff, direction) in criteria.items():
            val, tol = row[metric], tolerances[metric]
            strict = val >= cutoff if direction == "higher" else val <= cutoff
            eps = 1e-4
            near = val >= cutoff - tol - eps if direction == "higher" else val <= cutoff + tol + eps
            results.append((strict, near))

        if all(x[0] for x in results) or (sum(x[0] for x in results) == 3 and all(x[1] for x in results)):
            return tier

    return "Weakest"

best_models['model_assessment'] = best_models.apply(assess_model, axis=1)
...
for i in range(len(best_models)):
    ...
    model_assessment = best_models['model_assessment'].iloc[i]
    root_folder = f"./models/training/{best_models['response_var'].iloc[i]}/"
    full_fit_folder = None
    test = pd.read_parquet(Path(f"./ml_ready_data/test/{response}.parquet"))
    X_test = test.drop(columns = [response], errors = 'ignore')
    Y_test = test[response].astype(int)
    preds = get_prediction_by_model_type(response=response, model_type=model_type, root_folder=root_folder, new_data=X_test, extra_calibration=extra_calibration)
    ...
    assessments = pd.concat([assessments, assess_probability_ranges(calculate_probability_ranges(preds_df))[['ProbabilityFloor', 'ProbabilityCeiling', 'Assessment', 'ProbabilityRange', 'PctWin', 'n', 'MeanPrediction']].assign(
            ...
            training_root_folder=root_folder,
            full_fit_root_folder = full_fit_folder,
            model_assessment = model_assessment
        )], axis = 0, ignore_index = True)
assessments = assessments.rename(columns = {'Assessment': 'bin_assessment'}).drop(columns = ['ProbabilityRange'])
