...
def get_optimal_portfolio(df, model_lookup_table, correlations, min_stake = 0.5, max_bets = None, penalize_by_model = True, penalize_by_bin = True, apply_ev_caps = False, return_all_portfolios = False):
    ...
    df = calculate_ev_and_risk(df, model_lookup_table, penalize_by_model, penalize_by_bin, positive_only = True, apply_ev_caps = apply_ev_caps)
    ...
    for w in np.unique(df['Week']):
        ...
        mu, Sigma = get_mu_sigma(df_this_week, correlations)
        ...
        if len(mu) == 1:
            ...
            selected_rows['Portfolio_Sharpe'] = selected_rows['Portfolio_Mu'].iloc[0] / selected_rows['Portfolio_SD'].iloc[0] if selected_rows['Portfolio_SD'].iloc[0] > 0 else np.nan
            selected_rows['portfolio_id'] = 0
            df_with_calculations.append(selected_rows)
            continue
        start_weights = datetime.now()
        weights = calculate_weights(mu, Sigma, max_bets)
        ...
        if not return_all_portfolios:
            best_portfolio = get_portfolios_from_weights(full_weights_list, True)
            if best_portfolio is not None:
                df_with_calculations.append(best_portfolio)
        else:
            all_portfolios = get_portfolios_from_weights(full_weights_list, False)
            if len(all_portfolios) > 0:
                df_with_calculations.append(all_portfolios)
        print(f"Week {w} total: {(datetime.now() - start_week).seconds/60} minutes")
