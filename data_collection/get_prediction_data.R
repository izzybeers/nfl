...
player_bios = readRDS('data_collection/saved_data_files/player_bios.rds') #this likely won't change mid-week, so we can use the existing one
...
if(mid_week == TRUE)
{
  ...
  new_week_player_gamelogs_raw = readRDS(paste0('model/prediction_results/raw_data/',this_season,'_wk', this_week, '_player_gamelogs_df.rds'))
...
#things that get updated multiple times throughout the week:
...
if(mid_week == FALSE)
{
  ...
  {
    playoff_clinching_data_this_week = cbind(playoff_clinching_data_this_week %>% select(-Team), Team = unique(team_gamelogs_this_week$Team)) %>% select(Season, Week, Team, everything())
  }
} else {
...
if(mid_week == FALSE)
{
  ...
  #check this, it looked off after week 10:
  ...
  saveRDS(playoff_clinching_data_this_week, paste0('model/prediction_results/raw_data/', this_season,'_wk',this_week, '_playoff_clinching_df.rds'))
...
if(mid_week == FALSE)
{
  ...
  #update the prediction df and raw data files if predictions have already been run at some point this week
