...
gamelogs_df = gamelogs_df %>% arrange(Week) %>%
        select(player_id, Name, Position, Season, Gtm, Week, Date, Team, Opp, Game_Location, Month, day_of_week, Win, Differential, Active, GS, everything())
...
columns_0_1 = c()
low_medians = c()
for (c in colnames(gamelogs_df))
{
  pct_nonmissing = mean(!is.na(gamelogs_df[,c]))
  unique_values = gamelogs_df %>% select(!!sym(c)) %>% pull() %>% unique()
  median = gamelogs_df %>% select(!!sym(c)) %>% pull() %>% median(na.rm =TRUE)
  mean = gamelogs_df %>% select(!!sym(c)) %>% pull() %>% mean(na.rm = TRUE)
...
colnames(gamelogs_df)[which(colnames(gamelogs_df) == 'Avg_Win')]  = 'Pct_Win'
colnames(gamelogs_df)[which(colnames(gamelogs_df) == 'Avg_GS')]  = 'Pct_GS'
colnames(gamelogs_df)[which(colnames(gamelogs_df) == 'Avg_Active')]  = 'Pct_Active'
